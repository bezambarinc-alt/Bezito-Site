...
│       ├── products/      # Product grid (Zoho CRM cache)
...
│   ├── cron/pim-sync      # Zoho CRM → Neon (every 4h)
...
- All critical vars are set in Vercel production: `DATABASE_URL`, `JWT_SECRET`, `ZOHO_CLIENT_ID`, `ZOHO_CLIENT_SECRET`, `ZOHO_REFRESH_TOKEN`, `CRON_SECRET`, `APP_URL`, `BEZITO_SECRET`, `TRACK_SECRET`, `CLOUDINARY_*`, `ADMIN_*`
...
| Table | Purpose |
|---|---|
| `products` | Zoho CRM read cache — **never write manually**, rebuilt by pim-sync cron |
...
## Product Sync (Zoho CRM → Neon)

`GET /api/cron/pim-sync` — runs every 4h via Vercel cron (`vercel.json`).

- Authenticates with Zoho CRM OAuth (`ZOHO_CLIENT_ID`, `ZOHO_CLIENT_SECRET`, `ZOHO_REFRESH_TOKEN`)
- Lists all active products from Zoho CRM Products
- Derives SEO slugs from `Product_Name + category` via `deriveSlug()`
- Upserts into `products` table
- `active` and `featured` columns are **excluded from the upsert** — managed in admin only
- Deletes stale rows not returned by Zoho this run
- `revalidatePath('/jewelry', 'layout')` busts ISR cache after sync

Manual trigger: `GET /api/cron/pim-sync` with `Authorization: Bearer <BEZITO_SECRET>`.
