  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      mobileSlideRefs.current.forEach((slide, i) => {
        ...
        slide.style.transform = `translateY(calc(25vh + ${offset * 50}vh))`
        const absOff   = Math.abs(i - rounded)
        slide.style.filter  = absOff === 0 ? 'none' : 'blur(18px)'
        slide.style.opacity = absOff === 0 ? '1' : absOff === 1 ? '0.6' : '0'
      })

      // Only trigger React re-render when the displayed product changes
    ...
    <>
      ...
      >
        <div className={styles.mobilePin}>
          {/* Slides — transforms driven by scroll via rAF */}
          {products.map((p, i) => {
            const video = p.specs.heroVideoUrl
            const image = p.specs.heroPosterUrl
            return (
          ...
          {/* Top peek overlay — product identity + CTA */}
          ...
          {/* Bottom peek overlay — editorial lede */}
